import { Viewer } from "@apps/web/components/file-preview/types";

import { useState } from "react";
import { useDebouncedCallback } from "use-debounce";
import { toast } from "sonner";
import { Skeleton } from "@apps/web/components/file-preview/viewers/skeleton";
import { PDFJS, usePDFJS } from "@apps/web/hooks/use-pdfjs";
    ...
    <div className={"flex flex-row max-h-full overflow-hidden"}>
      <div className={"w-full overflow-y-scroll p-2 md:p-8"}>
        <div className="flex flex-col gap-4">
          {pages.map((src, i) => (
            <img key={i} src={src} className="max-w-full object-contain" />
          ))}
        </div>
      </div>
    </div>
