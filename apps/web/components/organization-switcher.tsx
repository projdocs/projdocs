...
import { AlertCircleIcon, Building, BuildingIcon, ChevronsUpDownIcon, EyeIcon, PlusIcon } from "lucide-react";
...
const Org = ({ current, disabled }: OrganizationSwitcherProps & {
  disabled?: boolean;
}) => (
  <>
    <div
  ...
  return (
    <SidebarMenu>
      <SidebarMenuItem>
        ...
        {!current.isAdmin && orgs?.length === 1 ? (
          <div className={cn(sidebarMenuButtonVariants({ variant: "default", size: "lg" }), "hover:bg-transparent hover:text-inherit")}>
            <Org disabled {...props} />
          </div>
        ) : (
          <DropdownMenu>
