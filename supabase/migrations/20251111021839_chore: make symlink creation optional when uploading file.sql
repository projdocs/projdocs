...
begin
    ...
    then
        ...
        if (new.user_metadata ->> 'directory_id') is null then
         -- do not create directory
        elsif not ((new.user_metadata ->> 'directory_id') ~* '^[0-9a-f-]{36}$') then
        ...
        if directory is not null and directory.id is not null then
