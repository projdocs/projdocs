import "client-only";
import { FilePreviewProps } from "@packages/ui/components/file-preview/types";
import { H2 } from "@packages/ui/components/typography";
import { useEffect, useState } from "react";
import { toast } from "sonner";
import { useDebouncedCallback } from "use-debounce";
import * as Viewers from "@packages/ui/components/file-preview/viewers";
import { ProjDocsAPI } from "@packages/shared/utilities/api";
...
export const FilePreview = (props: FilePreviewProps) => {
  ...
  useEffect(() => {
    ...
    if (!isTooLarge) ProjDocsAPI.from(supabase, props.apiURL).download(
      props.organization,
      props.file,
      props.version,
    ).then(({ data, error }) => {
