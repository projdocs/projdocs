...
CREATE OR REPLACE FUNCTION triggers.clients_before_actions()
    RETURNS trigger
    LANGUAGE plpgsql
    SECURITY DEFINER
    SET SEARCH_PATH = ''
...
begin
    if tg_op = 'INSERT' then
        ...
        NEW.number := triggers.next_number(
                _type := 'client'::triggers.organization_sequences,
...
CREATE TRIGGER before_actions
    ...
    ON public.clients
    FOR EACH ROW
EXECUTE FUNCTION triggers.clients_before_actions();